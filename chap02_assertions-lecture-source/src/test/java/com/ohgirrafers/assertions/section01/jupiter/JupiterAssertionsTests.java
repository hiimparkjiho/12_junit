package com.ohgirrafers.assertions.section01.jupiter;
...
class JupiterAssertionsTests {
    /*
    *  junit jupiter는 junit4로부터 온 assertion 메소드와 새롭게 자바 & 람다 표현식으로 추가된 메소드를 제공한다.
    *  모든 jupiter assertion의 메소드는 정적(static) 메소드 형태로 제공하고 있다.
    * */
...
//        Assertions.assertEquals(expected, result, "실패하는 경우 이게 보임");
        ...
        //Assertions.assertEquals(expected, result, () -> "실패");
    ...
    void testAssertType(){  // 테스트 코드는 외부에서 호출할 일이 없기 때문에 사용하지 않는다
        // given: 테스트 할 때 필요한 파라미터
        String firstName = "길동";
        String lastName ="홍";
        ...
        Assertions.assertAll(
                () -> Assertions.assertInstanceOf(Person.class, person),    // Person 타입이 맞는지 확인
    ...
    /*
    * assertDoesNotThrow(excutable) 메소드 호출시 아무런 예외가 발생하지 않는지 확인
    * */
...
//        );
    ...
    void TestAssertLinesMatch(){
        // given
        List<String> expected = Arrays.asList("java", "database", "spring");    // 배열로 반환해준
